...
class Queue extends EventEmitter {
  ...
  encode (queue, func, args = [], id = undefined) {
  ...
  async enqueue (q, func, args) {
    ...
    await this.connection.redis.rpush(this.connection.key('queue', q), this.encode(q, func, args, id))
    await PluginRunner.RunPlugins(this, 'afterEnqueue', func, q, job, args, id)
    return toRun
  }
