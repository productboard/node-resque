...
class Worker extends EventEmitter {
  ...
  async perform (job) {
    ...
    try {
      toRun = await PluginRunner.RunPlugins(this, 'beforePerform', job['class'], this.queue, this.jobs[job['class']], job.args, job.id)
      ...
      toRun = await PluginRunner.RunPlugins(this, 'afterPerform', job['class'], this.queue, this.jobs[job['class']], job.args, job.id)
      return this.completeJob(true)
    } catch (error) {
      this.error = error
      if (!triedAfterPerform) {
        try {
          await PluginRunner.RunPlugins(this, 'afterPerform', job['class'], this.queue, this.jobs[job['class']], job.args, job.id)
        } catch (error) {
